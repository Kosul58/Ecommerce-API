...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItemClass = (key: Section) =>
    `w-full flex items-center gap-3 p-3 rounded-lg transition-all duration-200 ease-in-out
      ${
        isActive(key)
  ...
  const subItemClass = (key: Section) =>
    `w-full text-left pl-10 pr-3 py-2 rounded-lg transition-colors duration-200
      ${
        isActive(key)
  ...
  return (
    <aside
      className={`relative h-screen bg-white border-r border-gray-200 shadow-xl flex flex-col justify-between
    ...
    >
      <div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-2">
          <button
            onClick={() => handleMenuItemSelect("sales")}
            ...
            >
              ...
              {collapsed &&
                (openCollapsedMenu === "products" ? (
                  <FaChevronUp className="text-xs absolute right-3" />
                ) : (
                  <FaChevronDown className="text-xs absolute right-3" />
                ))}
            </button>
      ...
      {collapsed &&
        ...
        popoutMenuTop !== null && (
          <div className={collapsedMenuClass} style={{ top: popoutMenuTop }}>
            <button
              onClick={() => handleMenuItemSelect("addProduct")}
              className="block w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-indigo-100 hover:text-indigo-700 rounded-md"
            >
              Add Product
            </button>
            <button
